...
export async function forgeScript(
  ...
  try {
    ...
    await $`forge clean`.cwd("lib/evvm/testnet").quiet();

    await $`${command}`.cwd("lib/evvm/testnet");
...
export async function showDeployContractsAndFindEvvm(
  ...
  const broadcastFile = `./lib/evvm/testnet/broadcast/Deploy.s.sol/${chainId}/run-latest.json`;
...
export async function showAllCrossChainDeployedContracts(
  ...
  startGettingDeployedContractsAnimation();
  const broadcastFileHost = `./lib/evvm/testnet/broadcast/DeployCrossChainHost.s.sol/${chainIdHost}/run-latest.json`;
  const broadcastContentHost = await Bun.file(broadcastFileHost).text();
  const broadcastJsonHost = JSON.parse(broadcastContentHost);

  const broadcastFileExternal = `./lib/evvm/testnet/broadcast/DeployCrossChainExternal.s.sol/${chainIdExternal}/run-latest.json`;
...
export async function contractInterfacesGenerator() {
  ...
  const fs = require("fs");
  const path = "./lib/evvm/testnet/src/interfaces";
  ...
  if (generateOnlyOne) {
    ...
    const evvmInterface =
      await $`cast interface lib/evvm/testnet/src/contracts/${folderName}/${contractName}.sol`.quiet();
    const interfacePath = `./lib/evvm/testnet/src/interfaces/I${contractName}.sol`;
    ...
    for (const contract of contracts) {
      ...
      startGeneratingInterfacesAnimation();
      let evvmInterface =
        await $`cast interface lib/evvm/testnet/src/contracts/${contract.folderName}/${contract.contractName}.sol`.quiet();
      let interfacePath = `./lib/evvm/testnet/src/interfaces/I${contract.contractName}.sol`;
...
export async function contractTesting() {
  ...
  if (print === "json" || print === "markdown") {
    const path = "./lib/evvm/testnet/output/testResults";

    await checkDirectoryPath(path);

    startTestAnimation();

    const result = await $`${command}`.cwd("lib/evvm/testnet").quiet();
